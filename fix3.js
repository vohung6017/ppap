        ...
        btnReject.addEventListener('click', function () {
            return withDisabled(btnReject, async () => {
                ...
                if (window.Swal) {
                    ...
                    // Get the current z-index of the task detail modal
                    const modalZIndex = modal ? parseInt(window.getComputedStyle(modal).zIndex || '1050', 10) : 1050;
                    ...
                    const result = await Swal.fire({
                        ...
                        focusConfirm: false,
                        didOpen: () => {
                            // Ensure SweetAlert modal appears above the task detail modal
                            const swalContainer = document.querySelector('.swal2-container');
                            if (swalContainer) {
                                swalContainer.style.zIndex = modalZIndex + 100;
                            }
                        }
                    });
                    if (!result.isConfirmed) return;
